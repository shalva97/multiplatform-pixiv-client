...
import io.github.shalva97.auth.*
import io.github.shalva97.tool.CaptchaHandler
...
private fun account(html: String): HtmlAccount {
    ...
    return io.github.shalva97.PixivJson.decodeFromString(HtmlAccount.serializer(), data)
}

/**
 * @param link for [POST_REDIRECT_URL]
 */
private suspend fun io.github.shalva97.PixivAuthClient.redirect(link: Url): String {
    val redirect = useHttpClient { it.get(link) }
    val location = redirect.request.url

    val account = io.github.shalva97.account(html = redirect.body())
 ...
 */
public suspend fun io.github.shalva97.PixivAuthClient.sina(show: suspend (qrcode: Url) -> Unit): AuthResult =
    login { redirect ->
    /**
     * for [LOGIN_URL]
     */
    val account = io.github.shalva97.account(html = useHttpClient { it.get(redirect).body() })
    ...
    // 为了直接继承参数 ↓
        val qrcode = io.github.shalva97.PixivJson.decodeFromString(WeiboQrcode.serializer(), useHttpClient { client ->
        client.get {
            takeFrom(temp.request)
            url {
                encodedPath = Url(io.github.shalva97.WEIBO_QRCODE_GENERATE).encodedPath
            }
        }.body()
    })
    ...
    supervisorScope {
        ...
        val jump: String = withTimeout(io.github.shalva97.WEIBO_QRCODE_TIMEOUT) {
        while (isActive) {
            delay(io.github.shalva97.WEIBO_QRCODE_INTERVAL)
            val status =
                io.github.shalva97.PixivJson.decodeFromString(WeiboQrcodeStatus.serializer(), useHttpClient { client ->
                    client.get(io.github.shalva97.WEIBO_QRCODE_QUERY) {
 ...
 */
public suspend fun io.github.shalva97.PixivAuthClient.cookie(load: () -> List<Cookie>): AuthResult = login { redirect ->
 ...
 */
public suspend fun io.github.shalva97.PixivAuthClient.password(
    id: String,
    pwd: String,
    ...
    val account = io.github.shalva97.account(html = login.body())
